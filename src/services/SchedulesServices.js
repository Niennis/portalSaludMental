...
export const fetchSchedules = async () => {
  const SCHEDULES_URL = import.meta.env.VITE_SCHEDULES_API + `/api/schedules`
...
export const fetchSchedule = async (id) => {
  const SCHEDULES_URL = import.meta.env.VITE_SCHEDULES_API + `/api/schedules/${id}`
...
export const deleteSchedule = async (id) => {
  const SCHEDULES_URL = import.meta.env.VITE_APPOINTMENTS_API + `/api/schedules/${id}`
  const data = await fetch(SCHEDULES_URL, {
    method: "DELETE",
    headers: {
      'content-type': 'application/json',
